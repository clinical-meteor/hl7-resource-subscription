...
Subscriptions.prepForFhirTransfer = function (subscription) {
  ...
  // to support situations where a family member might report on a subscription's
  // date of birth, but not know the year of birth; and the other way around
  if (subscription.birthDate) {
    subscription.birthDate = moment(subscription.birthDate).format("YYYY-MM-DD");
  }
  ...
  if (subscription.contact && subscription.contact[0]) {
    ...
    subscription.contact.forEach(function(contact){
      ...
      if (contact.name && contact.name.resourceType) {
        //console.log("subscription.contact.name", contact.name);
  ...
  //console.log("Subscriptions.prepForBundle()", subscription);
