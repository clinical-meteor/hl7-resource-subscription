...
export const insertSubscription = new ValidatedMethod({
  ...
  run(document) {
    ...
    process.env.TRACE && console.log("insertSubscription", document);
    ...
    return Subscriptions.insert(document);
  }
});

export const updateSubscription = new ValidatedMethod({
  ...
  run({ _id, update }) {
    process.env.TRACE && console.log("updateSubscription");
    process.env.TRACE && console.log("_id", _id);
    process.env.TRACE && console.log("update", update);

    // update = convertBirthdateToValidDate(update);

    let consent = Subscriptions.findOne({_id: _id});
    ...
    console.log("diffedSubscription", consent);
    return Subscriptions.update({_id: _id}, { $set: consent });
  }
});

export const removeSubscriptionById = new ValidatedMethod({
  ...
  run({ _id }) {
    ...
    return Subscriptions.remove({_id: _id});
  }
});
